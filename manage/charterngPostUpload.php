...
<?php
	...
	/*
	 * Charter NG - Insert a ZIP metadata after an FTP upload
	 *
	...
	if (!$_SERVER['argv'][1]) {
	    echo "\n    Usage : " . $_SERVER['argv'][0] . " [ZIP File]\n"; 
	    exit;
	}

	$fileName = $argv[1];
	$zip = CHARTERNG_HOME . "/archives/" . basename($fileName);
	
	// Copy uploaded file to the archives directory
	copy($fileName, $zip);
    ...
    if (unzip($srcDir . $zip, $targetDir, true, true)) {
        ...
        switch ($format) {
            ...
            /*
             * KOMPSAT-2 (K2) is DIMAP, i.e. means 3 files
             *      METADATA.DIM (metadata XML file)
             *      ICON.JPG (thumbnail)
             *      PREVIEW.JPG (quicklook)
             */
